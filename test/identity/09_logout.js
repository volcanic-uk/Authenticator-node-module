...
describe('logout the identity', () => {
    describe('with auth', async () => {
        it('should logout an identity', async () => {
            axiosVCR.mountCassette('./test/cassettes/identities/logout/identity_logout.json');
            let logout = await new Identity().withAuth().logout();
            axiosVCR.ejectCassette('./test/cassettes/identities/logout/identity_logout.json');
    ...
    describe('without auth and with setToken', async () => {
        ...
        it('should logout an identity', async () => {
            axiosVCR.mountCassette('./test/cassettes/identities/logout/identity_logout_auth.json');
            let logout = await new Identity().setToken(token).logout();
            axiosVCR.ejectCassette('./test/cassettes/identities/logout/identity_logout_auth.json');
            expect(logout).to.be.an('object');
        });

        it('should not logout an already logged out identity', async () => {
            axiosVCR.mountCassette('./test/cassettes/identities/logout/identity_logout_auth_fail.json', true);
            try {
            ...
            axiosVCR.ejectCassette('./test/cassettes/identities/logout/identity_logout_auth_fail.json');
        });
