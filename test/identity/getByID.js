...
describe('get identity by id', () => {
    ...
    describe('with auth', async () => {
        it('get an identity by id', async () => {
            axiosVCR.mountCassette('./test/cassettes/identity_get_by_id.json');
            await new Identity().withAuth().getByID(identityCreation.secure_id);
            axiosVCR.ejectCassette('./test/cassettes/identity_get_by_id.json');
        ...
        it('should not proceed if the identity does not exist upon read request, and it will throw an error', async () => {
            axiosVCR.mountCassette('./test/cassettes/fail_identity_getById.json', true);
            try {
            ...
            axiosVCR.ejectCassette('./test/cassettes/fail_identity_getById.json');
        });
        ...
        it('should fail when requesting a read request when there is no token in the request header, and it will throw an error', async () => {
            axiosVCR.mountCassette('./test/cassettes/fail_identity_token.json', true);
            try {
            ...
            axiosVCR.ejectCassette('./test/cassettes/fail_identity_token.json');
        });
