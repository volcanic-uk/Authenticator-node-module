...
describe('identity create tests', () => {
    ...
    describe('without auth and with setToken', async () => {
        ...
        it('creating a new identity with secret', async () => {
            let identityCreationWithSecret = await new Identity().setToken(loginToken).create(tmpIdentityNameWithSecret + '-withtoken', tmpIdentitySecret, principalID);
            expect(identityCreationWithSecret).to.be.an('object');
        });

        it('should not create a duplicate identity record', async () => {
            try {
                await new Identity().setToken(loginToken).create(tmpIdentityNameWithSecret, tmpIdentitySecret, principalID);
                throw Error('The code should not reach this scope as it would be a duplicate identity record');
            } catch (e) {
                expect(e.errorCode).to.equal(1003);
                expect(e).to.exist;
...
describe('identity update', async () => {
    ...
    describe('without auth and with setToken', async () => {
        ...
        it('should update an identity', async () => {
            let updatedIdentity = await new Identity().setToken(loginToken).update(`updated-name-${currentTimestampSecond}`, identityCreation.id);
            expect(updatedIdentity.name).to.equal(`updated-name-${currentTimestampSecond}`);
        });
...
describe('reset identity secret', async () => {
    ...
    describe('without auth and with setToken', async () => {
        ...
        it('should reset identity secret', async () => {
            let resetSecret = await new Identity().setToken(loginToken).resetSecret(`new-secret-${currentTimestampSecond}`, identityCreation.id);
            expect(resetSecret.message).to.equal('Secret regenerated successfully');
        });

        it('should generate identity secret', async () => {
            let generateSecret = await new Identity().setToken(loginToken).resetSecret(null, identityCreation.id);
            expect(generateSecret.message).to.equal('Secret regenerated successfully');
        });
...
describe('generate token for identity', async () => {
    ...
    describe('without auth and with setToken', async () => {
        ...
        it('should generate token for identity', async () => {
            let identity = await new Identity().setToken(loginToken).generateToken(identityCreation.id, ['*'], Math.round(new Date(today.getFullYear(), today.getMonth() + 2, today.getDate()).getTime()), false, Math.round(new Date(today.getFullYear(), today.getMonth(), today.getDate() + 1).getTime()));
            expect(identity.token).to.exist;
        });

        it('should generate token for identity when expiry date is in the past', async () => {
            try {
                await new Identity().setToken(loginToken).generateToken(identityCreation.id, ['*'], Math.round(new Date(today.getFullYear(), today.getMonth(), today.getDate()).getTime()), false, Math.round(new Date(today.getFullYear(), today.getMonth(), today.getDate() + 1).getTime()));
            } catch (e) {
                expect(e.errorCode).to.be.equal(1008);
                expect(e.message).to.exist;
            }
        ...
        it('should generate token for identity when nbf is more than 4 weeks', async () => {
            try {
                await new Identity().setToken(loginToken).generateToken(identityCreation.id, ['*'], Math.round(new Date(today.getFullYear(), today.getMonth() + 2, today.getDate()).getTime()), false, Math.round(new Date(today.getFullYear(), today.getMonth() + 4, today.getDate()).getTime()));
            } catch (e) {
                expect(e.errorCode).to.be.equal(1009);
                expect(e.message).to.exist;
            }
