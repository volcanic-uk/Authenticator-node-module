...
describe('principal create test', () => {
    ...
    it('should fail if principal creation function is called without a valid token and it will throw an error', async () => {
        axiosVCR.mountCassette('./test/cassettes/principals/create/principal_create_fail.json', true);
        try {
        ...
        axiosVCR.ejectCassette('./test/cassettes/principals/create/principal_create_fail.json');
    });

    it('should be a success when passing valid data, hence it will return an object carrying the created principal data', async () => {
        axiosVCR.mountCassette('./test/cassettes/principals/create/principal_create.json');
        ...
        axiosVCR.ejectCassette('./test/cassettes/principals/create/principal_create.json');
    });

    it('should not create a principal and it will throw an error if the name already exist', async () => {
        axiosVCR.mountCassette('./test/cassettes/principals/create/principal_create_fail_duplicate.json', true);
        try {
        ...
        axiosVCR.ejectCassette('./test/cassettes/principals/create/principal_create_fail_duplicate.json');
    });
});