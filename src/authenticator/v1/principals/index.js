...
class Principal extends V1Base {
     ...
     * @returns {Promise<*|{status}|undefined>}
     */
    async getIdentities({ page = 1, page_size = 10, query = '', name = '', source = '', dataset_id = null, sort = 'id', order = 'asc', id = null }) {
        return super.fetch('get', `principals/${id}/identities?query=${query}&dataset_id=${dataset_id}&page=${page}&${page_size}&sort=${sort}&order=${order}&name=${name}&source=${source}`, null, {});
    }
